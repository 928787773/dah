...
/**
 * route : api-form-question-query
 *
 * @author : QRQY
 * @date : 2021-06-24 16:20
 */

@Service
@Slf4j
@Validated
@Api(value = "问卷问题", tags = {"管理端", "问卷问题", "这里放筛选标签"})
public class ApiFormQuestionQueryService implements IBaseService<FormQuestionQueryQO> {
    @Autowired
    ...
    public ICommonResult execute(@Valid FormQuestionQueryQO qo, HttpServletRequest request, IBaseUser curUser, String version) {
        ...
        Specification query = new BaseMysqlQuery(qo)
                .append("status", "1").append("level", 0)
        ...
        List<FormQuestionListDTO> content = page.getContent().stream().map(t -> {
            ...
            FormQuestionTypeEnEnum enType= FormQuestionTypeEnEnum.parse(t.getType().name());
            dto.setEnType(enType);

            if(dto.getType() != FormQuestionTypeEnum.FILLIN){
                //查看选项
                List<FormQuestionOptionListDTO> formQuestionOptionList = this.getFormQuestionOptionList(dto.getId(),dto.getType());
     ...
     * @return
     */
    public List<FormQuestionOptionListDTO> getFormQuestionOptionList(Integer formQuestionId,FormQuestionTypeEnum formQuestionType){
        //1 该问题的选项
        ...
        List<FormQuestionOptionListDTO> content = formQuestionOptionList.stream().map(t -> {
            ...
            //2.2 处理自己问题格式
            if(!formQuestionType.equals(FormQuestionTypeEnum.SCORE)){
                    ...
                    childQuestionList.forEach(it->{
                        ...
                        if (childFormQuestionOptionMap.containsKey(it.getId())) {
                            ...
                            List<FormQuestionOptionListDTO> result = childFormQuestionOptionList.stream().map(child -> {
                                ...
                                questionOptionListDto.setItemFormQuestionList(new ArrayList<>());
