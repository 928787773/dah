...
def generateEntity(table, dir) {
    def className = camelCase(table.getName(), true) + "Collection"
...
import com.qrqy.dp.mongo.IBaseMongoRepository;
 ...
 */
@Repository
public interface ${className} extends IBaseMongoRepository<${entityClassName}, String> {
...
import com.qrqy.dp.mongo.BaseMongoManager;
import lombok.Data;
 ...
 */
@Component
public class ${className} extends BaseMongoManager<${repositoryClassName}, ${entityClassName}> {
    @Autowired
    ...
    //todo 可在这里增加业务方法，不限于 BaseMongoManager 提供的基础方法
...
def calcFields(table) {
    def idFound = 0;
    List fieldList = DasUtil.getColumns(table).reduce([]) { fields3, col ->
